...
//     let mut res = self.send(self.req_json(Method::GET, "/v3/orders/released", qs)?)?;
...
//     let mut res = self.send(self.req_json(
//       Method::GET,
//       "/v3/orders",
...
//     let mut res = self.send(self.req_json(Method::GET, &path, ())?)?;

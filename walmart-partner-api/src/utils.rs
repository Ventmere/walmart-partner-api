...
impl<'de> de::Visitor<'de> for TimestampVistor {
  ...
  fn visit_u64<E>(self, v: u64) -> Result<Self::Value, E>
    where E: de::Error
  {
    Utc.timestamp_opt((v / 1000) as i64, ((v % 1000) * 1000000) as u32)
      .single()
      .ok_or_else(|| E::custom(format!("invalid timestamp value `{}`", v)))
  }
}

pub fn deserialize_timestamp<'de, D>(d: D) -> Result<DateTime<Utc>, D::Error> where D: Deserializer<'de> {
...
mod tests {
  ...
  fn test_deserialize_timestamp() {
    ...
    assert_eq!(t.date.format("%Y-%m-%d").to_string(), "2017-08-12".to_string());
  }
}
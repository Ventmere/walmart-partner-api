...
macro_rules! impl_from_csv {
  (
    ...
    impl $name {
      fn from_csv(headers: &[String], row: &[&str]) -> Result<Self> {
        $(
          let mut $field = None;
        )*
        ...
        for (i, v) in row.into_iter().enumerate() {
          match headers.get(i).map(AsRef::as_ref) {
            $(
              Some($header) => {
                $field = v.parse().map_err(|err| -> Error {
...
impl ReportType for ItemReportType {
  ...
  fn deserialize<R: Read>(mut r: R) -> Result<Self::Data> {
    ...
    let headers: Vec<String> = csv_reader.headers().map_err(|err| -> Error {
        format!("Parse csv header error: {}", err).into()
      })?
      .iter()
      .map(|s| s.to_string())
      .collect();
    ...
    for (i, result) in csv_reader.records().enumerate() {
      let record = result.map_err(|err| -> Error {
